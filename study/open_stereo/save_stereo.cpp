...
void stereoCallback(const xv::FisheyeImages& stereo) {
    ...
    cv::Mat left(stereo.images[1].height, stereo.images[1].width, CV_8UC1, const_cast<uint8_t*>(stereo.images[1].data.get()));
    cv::Mat right(stereo.images[0].height, stereo.images[0].width, CV_8UC1, const_cast<uint8_t*>(stereo.images[0].data.get()));
    ...
    // 保存左右图像为 PNG 文件
    std::string leftFilename = "left_image_" + std::to_string(frameCount) + ".png";
    std::string rightFilename = "right_image_" + std::to_string(frameCount) + ".png";
    
    // 保存图像
    cv::imwrite(leftFilename, left);
    cv::imwrite(rightFilename, right);
    
    std::cout << "Saved Left Image: " << leftFilename << std::endl;
    std::cout << "Saved Right Image: " << rightFilename << std::endl;
